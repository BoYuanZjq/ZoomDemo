...
/**
 ...
 @param strRTCPeerId RTC服务生成的标识Id
 @param bAudio YES:传输;NO:不传输
 @param bVideo YES:传输;NO:不传输
 @return 是否成功
 */
- (BOOL)setPeerAVEnable:(NSString*)strRTCPeerId audio:(bool)bAudio video:(bool)bVideo;
...
/**
 ...
 说明：默认：AnyRTCVideoRenderScaleAspectFill，等比例填充视图模式
 */
- (void)updateRTCVideoRenderModel:(AnyRTCVideoRenderMode)eVideoRenderMode pubId:(NSString*)strRTCPubId;
...
#pragma mark - ZOOM模式
/**
 设置显示模式（必须先设置会议模式为zoom模式才有效）

 @param eType 模式：默认为AnyZoomTypeSingle模式
 @return 成功与否
 */
- (BOOL)setZoomModel:(AnyZoomType)eType;

/**
 设置显示页码（分屏显示AnyZoomTypeNomal）

 @param nPage 页码（从0开始，每页加上自己的视频流为4路）
 @return 成功与否
 */
- (BOOL)setZoomPage:(int)nPage;

/**
 设置显示区域从nIndex 到第几个

 @param nIndex 开始标记
 @param nShowNum 数量，默认为4：onRTCZoomPageInfo回调里的nShowNum
 @return 成功与否
 */
- (BOOL)setZoomPageIndex:(int)nIndex showNum:(int)nShowNum;
