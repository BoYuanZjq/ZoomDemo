...
typedef NS_ENUM(NSInteger,AnyMeetingType) {
    ...
    AnyMeetingTypeLive = 2,   // live模式：暂时未实现
    AnyMeetingTypeZoom = 3,   // zoom模式
};

//zoom模式
typedef NS_ENUM(NSInteger,AnyZoomType){
    AnyZoomTypeNomal = 0, // 一般模式:分屏显示模式
    AnyZoomTypeSingle = 1, // 单显示模式:语音激励模式
    AnyZoomTypeDriver = 2, // 驾驶模式:只接受音频:此时自己设置音视频是否传输
};
...
/**
 视频会议最大人数(默认为4人，根据自己需求更改，zoom模式可以为64人，其他模式建议不要超过9人)
